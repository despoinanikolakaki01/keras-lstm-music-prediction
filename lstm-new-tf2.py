...
def get_notes():
	...
	for file in glob.glob("midis/classical-piano-type0/*.mid"):
	...
	with open('durations', 'wb') as filepath:
		pickle.dump(durations, filepath)
		
	with open('offsets', 'wb') as filepath:
...
def train(model, network_input_notes, network_input_offsets, network_input_durations, network_output_notes, network_output_offsets, network_output_durations):
	...
	filepath = "weights_maestro/weights-improvement_new-{epoch:02d}-{loss:.4f}-bigger.hdf5"
	...
	model.fit([network_input_notes, network_input_offsets, network_input_durations], [network_output_notes, network_output_offsets, network_output_durations], epochs=200, batch_size=64, callbacks=callbacks_list, verbose=1)
